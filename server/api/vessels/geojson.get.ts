...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.warn('[api/vessels/geojson] Backend request error:', err?.message)
  }
  ...
  const vessels: NormalizedVessel[] = ships.map((ship) => {
    ...
    return {
      ...
      tone,
      hasCameraAi: p.has_camera_ai !== undefined ? Boolean(p.has_camera_ai) : (p.hasCameraAi !== undefined ? Boolean(p.hasCameraAi) : undefined),
    }
  })
