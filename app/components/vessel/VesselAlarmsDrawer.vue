...
<template>
  <aside
    v-if="isOpen"
    class="sticky top-[calc(3.5rem+72px)] h-[calc(100svh-3.5rem-72px)] w-[360px] xl:w-[390px] shrink-0 border-l border-border bg-card flex flex-col text-foreground z-10"
  >
    <!-- Top Drawer Header -->
    <div class="flex h-12 shrink-0 items-center justify-between border-b border-border px-4 bg-muted/20">
      <div class="flex items-center gap-2.5 min-w-0">
        ...
        <h2 class="font-bold text-sm tracking-tight text-foreground truncate">
          Alarms &amp; Events
        </h2>
      </div>
      ...
      <div class="flex items-center gap-1 shrink-0">
        ...
        <Button
          variant="ghost"
          size="icon"
          class="size-7 text-muted-foreground hover:text-foreground hover:bg-muted cursor-pointer rounded-md"
    ...
    <div class="px-4 py-2.5 border-b border-border bg-muted/10 space-y-2">
      <div class="flex items-center justify-between text-xs">
        <span class="flex items-center gap-1.5 text-rose-400 font-medium">
          ...
          <span class="font-bold font-mono">{{ criticalAlarmsCount }}</span> critical
        </span>
        <span class="flex items-center gap-1.5 text-amber-400 font-medium">
          <span class="size-1.5 rounded-full bg-amber-500" />
          <span class="font-bold font-mono">{{ warningAlarmsCount }}</span> warnings
        </span>
        <span class="flex items-center gap-1.5 text-sky-400 font-medium">
          <span class="size-1.5 rounded-full bg-sky-500" />
          <span class="font-bold font-mono">{{ infoAlarmsCount }}</span> advisory
        </span>
      </div>
    ...
    <div class="flex items-center border-b border-border bg-muted/20 px-2 py-1.5 gap-1 text-xs">
      <button
        type="button"
        @click="activeCategory = 'all'"
        :class="[
          'flex-1 h-6 text-xs font-medium rounded-md transition-all cursor-pointer text-center',
          activeCategory === 'all'
      ...
      <button
        ...
        :class="[
          'flex-1 h-6 text-xs font-medium rounded-md transition-all cursor-pointer text-center flex items-center justify-center gap-1',
    ...
    <div class="p-3 border-b border-border bg-background space-y-2.5">
      <!-- Search Input -->
      <div class="relative">
        ...
        <input
          ...
          class="h-8 w-full rounded-lg border border-border/50 bg-muted/25 pl-8 pr-7 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50"
        />
        <button
          v-if="searchQuery"
          type="button"
      ...
      <div class="flex items-center justify-between gap-1 text-xs">
        <div class="flex items-center gap-1 overflow-x-auto">
          <button
            ...
            :class="[
              'h-6 px-2 text-xs rounded-md border transition-colors cursor-pointer',
              activeSeverity === 'all'
          ...
          <button
            ...
            :class="[
              'h-6 px-2 text-xs rounded-md border transition-colors cursor-pointer flex items-center gap-1',
              activeSeverity === 'critical'
                ? 'border-rose-500/50 bg-rose-500/15 text-rose-400 font-medium'
                : 'border-border/50 text-muted-foreground hover:text-rose-400'
            ]"
          >
          ...
          <button
            ...
            :class="[
              ...
              activeSeverity === 'warning'
                ? 'border-amber-500/50 bg-amber-500/15 text-amber-400 font-medium'
                : 'border-border/50 text-muted-foreground hover:text-amber-400'
            ]"
          >
          ...
          <button
            ...
            :class="[
              'h-6 px-2 text-xs rounded-md border transition-colors cursor-pointer flex items-center gap-1',
              activeSeverity === 'info'
                ? 'border-sky-500/50 bg-sky-500/15 text-sky-400 font-medium'
                : 'border-border/50 text-muted-foreground hover:text-sky-400'
            ]"
          >
        ...
        <div class="flex items-center gap-1 shrink-0">
          <button
            ...
            :class="[
              'h-6 px-2 rounded-md border text-xs cursor-pointer transition-colors',
              hideAcknowledged
          ...
          <button
            ...
            class="h-6 px-2 rounded-md border border-border/50 text-xs text-muted-foreground hover:text-foreground hover:bg-muted cursor-pointer flex items-center gap-1"
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3">
      <div
        ...
        :key="item.id"
        :class="[
          'rounded-xl border p-3.5 transition-all flex flex-col gap-2.5 shadow-xs',
          item.acknowledged
            ? 'bg-muted/15 border-border/30 opacity-60'
            : item.level === 'critical'
              ? 'border-border/40 border-l-4 border-l-rose-500 bg-card hover:bg-muted/10'
              : item.level === 'warning'
                ? 'border-border/40 border-l-4 border-l-amber-500 bg-card hover:bg-muted/10'
                : 'border-border/40 border-l-4 border-l-sky-500 bg-card hover:bg-muted/10'
        ]"
      >
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span
              :class="[
                'inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-bold font-mono',
                item.level === 'critical'
            ...
            <span class="text-xs text-muted-foreground font-medium bg-muted/40 px-2 py-0.5 rounded border border-border/40">
              {{ item.system }}
            </span>
            ...
            <span
              ...
              class="inline-flex items-center gap-1 text-xs font-mono text-muted-foreground bg-muted px-1.5 py-0.5 rounded"
            >
        ...
        <!-- Alarm Title (Strengthened) -->
        <div class="text-sm font-bold text-foreground leading-snug">
          {{ item.title }}
        </div>

        <!-- Observed vs Limit Comparison Box (Shortened, High Contrast) -->
        <div
          v-if="item.reading || item.limit"
          class="flex items-center justify-between text-xs bg-muted/30 px-3 py-2 rounded-lg"
        >
          <div class="flex items-center gap-1.5 truncate">
            <span class="text-muted-foreground">Observed:</span>
            <span
              class="font-mono font-bold"
              :class="item.level === 'critical' ? 'text-rose-400' : item.level === 'warning' ? 'text-amber-400' : 'text-sky-400'"
            >
              {{ item.reading }}
            </span>
          </div>
          <div class="h-3 w-px bg-border/40 shrink-0" />
          <div class="flex items-center gap-1.5 truncate text-right">
            <span class="text-muted-foreground">Threshold:</span>
            <span class="font-mono font-medium text-muted-foreground">{{ item.limit }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-2 border-t border-border/30 mt-0.5">
          ...
          <Button
            ...
            size="sm"
            :class="[
              'h-6 px-2.5 text-xs gap-1.5 cursor-pointer font-medium rounded-md',
              item.acknowledged
                ? 'text-muted-foreground hover:text-foreground'
      ...
      <div v-if="filteredAlarms.length === 0" class="py-16 text-center space-y-2">
        <div class="size-10 rounded-full bg-muted/40 flex items-center justify-center mx-auto text-muted-foreground">
          <CheckCircle2 class="size-5 text-teal-400" />
        </div>
        <div class="text-sm font-bold text-foreground">No Alarms Found</div>
        <p class="text-xs text-muted-foreground max-w-[240px] mx-auto">
          {{ searchQuery ? 'No events match your search query.' : 'All events matching the selected category are cleared or acknowledged.' }}
        </p>
    ...
    <!-- Footer Strip -->
    <div class="p-3 border-t border-border bg-muted/20 flex items-center justify-between text-xs text-muted-foreground">
      <div class="flex items-center gap-2">
        <span class="size-2 rounded-full bg-teal-500" />
        <span class="font-mono text-xs">CBM Engine Live</span>
      </div>
      <button
        type="button"
